...
export function Button({ label, variant='primary', size='md', loading=false, disabled=false, icon, fullWidth=false, style, onPress, ...rest }: ButtonProps) {
  ...
  return (
    <TouchableOpacity onPress={onPress} disabled={isDisabled} activeOpacity={0.82} style={containerStyle} {...rest}>
      {isGradient
        ? <LinearGradient colors={isDisabled ? [Colors.grey300,Colors.grey300] : GRADIENTS[variant]} start={{x:0,y:0}} end={{x:1,y:0}} style={innerStyle}>{content}</LinearGradient>
